...
type VerificationCodePanelProps = {
  email: string;
  password?: string;
  onVerified?: (userId: string) => void | Promise<void>;
  onError?: (message: string) => void;
  className?: string;
  /** Compact styling inside success banners */
...
export function VerificationCodePanel({
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(data.error || t("login.verificationCodeInvalid"));
      }
      ...
      if (password) {
        ...
        await onVerified?.(signIn.data.user.id);
        return;
      }
  ...
  return (
    <div className={cn("space-y-3", className)}>
      <p className={cn("text-sm", inline && "opacity-90")}>{t("login.verifyCodePrompt")}</p>
      <div

...
function AdminAccessContent() {
  ...
  const returnTo = searchParams.get("return") || "/dashboard/admin";
  ...
  useEffect(() => {
    fetch("/api/admin/me")
      .then(async (res) => {
        if (res.status === 401) {
          router.replace(`/login?redirect=${encodeURIComponent("/admin-access")}`);
          return;
        }
          ...
          <Card className={`p-8 sm:p-10 border ${panel}`}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className={`text-sm ${muted}`}>The admin dashboard is restricted to platform administrators.</p>
              </div>
            </div>
...
WHERE email = '${me?.email || "your@email.com"}';`}
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Button variant="outline" href={`/dashboard/${me?.role || "student"}`}>
                  ...
                  >
                    Retry admin URL <ArrowRight className="w-4 h-4" />
                  </Link>
                </div>
              </>
            )}

...
import {
  getZoomConfig,
  getZoomOAuthEnvironment,
  isMultiUserOAuthReady,
  isZoomOAuthConfigured,
} from "@/lib/zoom/config";
...
export async function GET() {
  ...
  const [connectionsRes, lessonsRes, upcomingRes, activeRes, upcomingListRes, teachersRes] = await Promise.all([
    ...
    admin
      .from("lessons")
      .select("id, meeting_title, start_time, duration, meeting_type, zoom_link, teacher_id, student_id, status")
      .not("zoom_meeting_id", "is", null)
      .eq("status", "scheduled")
      .gte("start_time", nowIso)
  ...
  const connectedSet = new Set(
    (connectionsRes.data ?? []).map((c: { teacher_id: string }) => c.teacher_id)
  );

  const teachersWithZoomStatus = (teachersRes.data ?? []).map((t: { id: string; full_name: string | null; email: string | null }) => ({
    id: t.id,
    name: t.full_name,
    email: t.email,
    zoomConnected: connectedSet.has(t.id),
    zoomEmail: (connectionsRes.data ?? []).find((c: { teacher_id: string; zoom_email: string | null }) => c.teacher_id === t.id)?.zoom_email ?? null,
  }));

  return NextResponse.json({
    connectedTeachers: connectionsRes.count ?? 0,
    connections: connectionsRes.data ?? [],
    teachers: teachersWithZoomStatus,
    ...
    oauth: {
      ...
      redirectUri: getZoomConfig().redirectUri,
    },
  });
}

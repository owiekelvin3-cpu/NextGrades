-- =============================================================================
-- NextGrades Teacher CMS — run in Supabase SQL Editor
-- https://supabase.com/dashboard/project/pzavnfdhctsrhzesdvfd/sql/new
--
-- Safe to re-run. Fixes common errors from the original 00003 script:
--   • "relation already exists"
--   • "policy already exists"
--   • "duplicate key" on seed inserts
--   • duplicate download_count column (already on materials from 00001)
--
-- Prerequisite: materials table must exist (run 00001_initial_schema.sql first)
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Missing public.materials — run supabase/migrations/00001_initial_schema.sql first';
  END IF;
END $$;

-- Paste contents of 00003_teacher_cms.sql below (kept in sync with migration file)
...
CREATE POLICY "Everyone can view active categories" ON public.resource_categories FOR SELECT USING (is_active = TRUE);
DROP POLICY IF EXISTS "Admins can manage categories" ON public.resource_categories;
CREATE POLICY "Admins can manage categories" ON public.resource_categories FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "Everyone can view tags" ON public.resource_tags;
CREATE POLICY "Everyone can view tags" ON public.resource_tags FOR SELECT USING (TRUE);
DROP POLICY IF EXISTS "Admins can manage tags" ON public.resource_tags;
CREATE POLICY "Admins can manage tags" ON public.resource_tags FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "Everyone can view tag relations" ON public.resource_tag_relations;
CREATE POLICY "Everyone can view tag relations" ON public.resource_tag_relations FOR SELECT USING (TRUE);
DROP POLICY IF EXISTS "Admins and teachers can manage tag relations" ON public.resource_tag_relations;
CREATE POLICY "Admins and teachers can manage tag relations" ON public.resource_tag_relations FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND (role = 'admin' OR role = 'teacher')));
DROP POLICY IF EXISTS "Teachers can view analytics for their resources" ON public.resource_analytics;
CREATE POLICY "Teachers can view analytics for their resources" ON public.resource_analytics FOR SELECT USING (EXISTS (SELECT 1 FROM public.materials m WHERE m.id = resource_analytics.resource_id AND m.created_by = auth.uid()));
DROP POLICY IF EXISTS "Admins can view all analytics" ON public.resource_analytics;
CREATE POLICY "Admins can view all analytics" ON public.resource_analytics FOR SELECT USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "System can create analytics" ON public.resource_analytics;
CREATE POLICY "System can create analytics" ON public.resource_analytics FOR INSERT WITH CHECK (TRUE);
DROP POLICY IF EXISTS "Teachers can view their own folders" ON public.resource_folders;
CREATE POLICY "Teachers can view their own folders" ON public.resource_folders FOR SELECT USING (teacher_id = auth.uid());
DROP POLICY IF EXISTS "Teachers can manage their own folders" ON public.resource_folders;
CREATE POLICY "Teachers can manage their own folders" ON public.resource_folders FOR ALL USING (teacher_id = auth.uid());
DROP POLICY IF EXISTS "Admins can manage all folders" ON public.resource_folders;
CREATE POLICY "Admins can manage all folders" ON public.resource_folders FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "Teachers can manage their own materials" ON public.materials FOR ALL USING (created_by = auth.uid() OR EXISTS (SELECT 1 FROM public.resource_folders f WHERE f.id = materials.folder_id AND f.teacher_id = auth.uid()));
DROP POLICY IF EXISTS "Admins can manage all materials" ON public.materials;
CREATE POLICY "Admins can manage all materials" ON public.materials FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'));

INSERT INTO public.resource_categories (name, description, icon, sort_order)
SELECT v.name, v.description, v.icon, v.sort_order FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM public.resource_categories c WHERE c.name = v.name);
...
WHERE NOT EXISTS (SELECT 1 FROM public.resource_tags t WHERE t.slug = v.slug);
...
-- Verify
SELECT 'resource_categories' AS table_name, count(*)::int AS rows FROM public.resource_categories
UNION ALL SELECT 'resource_tags', count(*)::int FROM public.resource_tags;
